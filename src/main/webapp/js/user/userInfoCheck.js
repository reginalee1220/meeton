...
function idCheck(){
    ...
    var userid=$("#aka").val();
    //1.입력글자 길이 체크
    if($.trim($("#aka").val()).length < 5){
        var newtext='<font color="#bc3838" size="13px">5~20자리로만 사용 가능합니다.</font>';
        $("#akaError").text('');
        $("#akaError").show();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data==1){	//중복 닉네임
                var newtext='<font color="#bc3838" size="13px">중복 닉네임입니다.</font>';
                $("#akaError").text('');
                $("#akaError").show();
                $("#akaError").append(newtext);
                $("#aka").val('').focus();
                ...
                var newtext='<font color="#4da9de" size="13px">사용가능한 닉네임입니다.</font>';
                $("#akaError").text('');
                $("#akaError").show();
                $("#akaError").append(newtext);
                $("#passwd").focus();
